...
void GetOpacityFactor(out float val1, out float val2, out float val3, out float val4)
{
	...
	for (int i = 0; i < 6; ++i){
		for (int j = 0; j < 6; ++j){
			...
			val1 += fs_in.light_view_depth_1 > texture(depth_map, tex_pos).y + tolerance ? (1. - hair_shadow) * (1. - head_shadow) : pow(1. - hair_shadow, mix(opacity_range_1.x, opacity_range_1.y, ratio1));
		}
	}
	val1 /= 36.;
	...
	for (int i = 0; i < 6; ++i){
		for (int j = 0; j < 6; ++j){
			...
			val2 += fs_in.light_view_depth_2 > texture(depth_map, tex_pos).y + tolerance ? (1. - hair_shadow) * (1. - head_shadow) : pow(1. - hair_shadow, mix(opacity_range_2.x, opacity_range_2.y, ratio2));
		}
	}
	val2 /= 36.;
	...
	for (int i = 0; i < 6; ++i){
		for (int j = 0; j < 6; ++j){
			...
			val3 += fs_in.light_view_depth_3 > texture(depth_map, tex_pos).y + tolerance ? (1. - hair_shadow) * (1. - head_shadow) : pow(1. - hair_shadow, mix(opacity_range_3.x, opacity_range_3.y, ratio3));
		}
	}
	val3 /= 36.;
	...
	for (int i = 0; i < 6; ++i){
		for (int j = 0; j < 6; ++j){
			...
			val4 += fs_in.light_view_depth_4 > texture(depth_map, tex_pos).y + tolerance ? (1. - hair_shadow) * (1. - head_shadow) : pow(1. - hair_shadow, mix(opacity_range_4.x, opacity_range_4.y, ratio4));
		}
	}
	val4 /= 36.;
}
